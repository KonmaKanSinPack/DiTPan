...
class ExpSmooth(_Loss):
    def __init__(self):
        super(ExpSmooth, self).__init__()
        # self.a = 1/2*math.e - 1/2
        # self.b = 7/4 - 3/2*math.e - 1/4*math.pow(math.e,3) 写错了却谜之有效
        self.a = np.float64(1/(2*math.e) - 1/2)
        self.b = np.float64(7/4 -3/(2*math.e) - 1/(4*math.e**2))
        # print(self.a)
        # print(self.b)
        # print(math.pow((1+self.a),2)+self.b)
        # print(1+(1-math.e**-1))

    def forward(self,input: Tensor, target: Tensor) -> Tensor:
        res = torch.abs(target - input)
        cond = res < 1
        return torch.mean(torch.where(cond,res+(1-torch.exp(-res)),torch.pow((res+self.a),2)+self.b))
